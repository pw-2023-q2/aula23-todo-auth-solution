...
export class ItemController {
    ...
    newest:RequestHandler = async (req, res, next) => {
        try {
            const items = await this.dao.listAll(req.session.email || '')
    ...
    tags: RequestHandler = async (req, res, next) => {
        try {
            const groupedItems = groupByTags(await this.dao.listAll(req.session.email || ''))
